import random,sys,os,pygame,ctypes
from pygame.locals import *
pygame.init()
pygame.display.set_caption('Isaacs Very Nice SNEK game')
os.environ['SDL_VIDEO_WINDOW_POS'] = "0,32"
user32 = ctypes.windll.user32
Moniter = user32.GetSystemMetrics(0), user32.GetSystemMetrics(1)

GameWidth=500
GameHeight=500
StartScreenHeight = 570
...
ComputerScreen = []
ComputerScreen.append(Moniter[0])
ComputerScreen.append(Moniter[1] - (CELLSIZE * 6))
...
def makeFruits(CellWidth, CellHeight, GameHeightReached = False, GameWidthReached = False):
    x=random.randint(0, CellWidth - 1)
    y=random.randint(0, CellHeight - 1)
    Chance = random.randint(0,1001)

    if (Chance % 1000) == 0:
        Type = "Master"
    else:
        if GameWidthReached and GameHeightReached:
            TypeList=["Normal","Speed"]
        else:
...
def drawLengths(SnekCords,Side,windowSurface,GameWidth):
    if Side=="LEFT":
...
def drawStartScreen(Cords_list1,Cords_list2,Snek1,Snek2,Snek1GrowthApples,Snek2GrowthApples,Snek1SpeedApples,Snek2SpeedApples,Snek1BoardApples,Snek2BoardApples):
    ...
    titleSurf1 = BIGFONT.render('BRO TIME FOR A SNEK GAME', True, WHITE, None)
    ...
    StatsRect = pygame.draw.rect(LargeWindow,RED,(10,(StartScreenHeight-60),90,100))
    ControlsWords = FONT.render(" CONTROLS ",True,WHITE)
    ControlsRect = pygame.draw.rect(LargeWindow,RED,(10,(StartScreenHeight-30),90,100))
    ...
    while True:
        ...
        drawGrid(LargeWindow,StartScreenHeight,GameWidth)
        ...
        ControlsButton = LargeWindow.blit(ControlsWords,ControlsRect)
        pygame.display.flip()
        ...
        for event in pygame.event.get(): 
            if event.type == QUIT:
                ...
                if StatButton.collidepoint(MousePosition):
                    ...
                    drawControlsScreen(LargeWindow)
...
def drawControlsScreen(Window):
    WindowX = 10
    WindowY =  100
    WindowGap = 30
    #Messages to display:
    WindowOpener = MEDIUMFONT.render("CONTROLS WINDOW - ESC TO RETURN",True,RED)
    Snek1Keys = ALMOSTMEDIUMFONT.render("WASD To Control The BLUE Snek",True,BLUE) 
    ...
    while True:
        Window.fill(BLACK)
        drawGrid(Window,StartScreenHeight,GameWidth)
        Window.blit(WindowOpener,WindowOpenerRect)
        Window.blit(Snek1Keys,Snek1KeysRect)
        Window.blit(Snek2Keys,Snek2KeysRect)
        Window.blit(StartGame,StartGameRect)
        Window.blit(PauseGame,PauseGameRect)
        pygame.display.flip()
...
def drawPaused(Window,Width,Height):
    Words = BIGFONT.render("GAME IS PAUSED",True,RED)
    WordsRect = Words.get_rect()
    WordsRect.center = (int(Width/2),int(Height/2))
...
def Runner(AppleCords,Snake1Cords,Snake1Direction,Snake2Cords,Snake2Direction,TicksPerSec,GameWidth,GameHeight):
    ...
    GameHeightReached = False
    GameWidthReached = False
    ...
    while True:
        ...
        if not Paused:
            if not PlayerWon:
                ...
                if Snake1Cords[HEAD]['x'] == AppleCords['x'] and Snake1Cords[HEAD]['y'] == AppleCords['y'] and AppleCords["Type"] == "Normal":
                    AppleCords=makeFruits(CellWidth,CellHeight,GameHeightReached,GameWidthReached) # set a new apple somewhere
                    ...
                    if GameWidth <= ComputerScreen[0]:
                        GameWidth += CELLSIZE
                    else:
                        GameWidthReached = True
                    if GameHeight <= ComputerScreen[1]:
                        GameHeight += CELLSIZE
                    else:
                        GameHeightReached = True
        ...
        drawLengths(Snake1Cords,"LEFT",SmallWindow,GameWidth)
        drawLengths(Snake2Cords,"RIGHT",SmallWindow,GameWidth)
        drawSNEKS(Snake1Cords,"1",SmallWindow)
        drawSNEKS(Snake2Cords,"2",SmallWindow)
        drawFruits(AppleCords,SmallWindow)
        if Paused:
            drawPaused(SmallWindow,GameWidth,GameHeight)
